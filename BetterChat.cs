...
{
    [Info("Better Chat", "SeesAll", "5.2.16")]
    ...
    internal class BetterChat : CovalencePlugin
    {
        ...
        private readonly Dictionary<Plugin, Func<IPlayer, string>> _thirdPartyTitles = new Dictionary<Plugin, Func<IPlayer, string>>();

        private static readonly string[] _stringReplacements = new[]
        {
        ...
        private static readonly Regex[] _regexReplacements = new[]
        {
...
#endif
        {
            ...
            for (int i = 0; i < loadedPlugins.Length; i++)
            {
                ...
                if (hookResult is Dictionary<string, object> modifiedData)
                {
                    chatMessageDict = modifiedData;
                }
            ...
            ChatGroup.FormatSettings output = chatMessage.GetOutput();
            ...
            switch (chatchannel)
            {
                ...
                case Chat.ChatChannel.Cards:
                    ...
                    List<Network.Connection> cardConnections = Facepunch.Pool.Get<List<Network.Connection>>();
                    ...
                    foreach (CardPlayerData playerData in baseCardGame.GameController.PlayerData)
                    {
                        ...
                        BasePlayer cardPlayer = BasePlayer.FindByID(playerData.UserID);
                        if (cardPlayer?.net?.connection != null)
                            cardConnections.Add(cardPlayer.net.connection);
                    }

                    if (cardConnections.Count > 0)
                        ConsoleNetwork.SendClientCommand(cardConnections, "chat.add", (int)chatchannel, chatMessage.Player.Id, output.Chat);

                    Facepunch.Pool.FreeUnmanaged(ref cardConnections);
                    break;
                ...
                case Chat.ChatChannel.Clan:
                    ...
                    List<Network.Connection> clanConnections = Facepunch.Pool.Get<List<Network.Connection>>();
                    ...
                    if (ClanManager.ServerInstance.Backend?.TryGet(clanid, out clan) ?? false)
                    {
                        ...
                        {
                            BasePlayer clanPlayer = BasePlayer.FindByID(member.SteamId);
                            if (clanPlayer != null && clanPlayer.IsConnected && clanPlayer.net?.connection != null)
                                clanConnections.Add(clanPlayer.net.connection);
                        }
                    }

                    if (clanConnections.Count > 0)
                        ConsoleNetwork.SendClientCommand(clanConnections, "chat.add", (int)chatchannel, chatMessage.Player.Id, output.Chat);

                    Facepunch.Pool.FreeUnmanaged(ref clanConnections);
                    break;
            ...
            {
                if (!chatMessage.BlockedReceivers.Contains(player.Id))
                    player.Message(output.Chat);
            }
        ...
        private void CmdChat(IPlayer player, string cmd, string[] args)
        {
            ...
            var commands = new Dictionary<string, Action<string[]>>
            {
                ...
                ["group set"] = a =>
                {
                    if (a.Length != 3)
                    {
                        ...
                        player.Reply($"Fields:{Environment.NewLine}{string.Join(", ", ChatGroup.Fields.Select(kvp => $"({kvp.Value.UserFriendyType}) {kvp.Key}"))}");
                        return;
                    }
                    ...
                    switch (group.SetField(field, strValue))
                    {
                        ...
                        case ChatGroup.Field.SetValueResult.InvalidValue:
                            player.ReplyLang("Invalid Value", new Dictionary<string, string>
                            {
                                ["field"] = field,
                                ["value"] = strValue,
                                ["type"] = ChatGroup.Fields[field].UserFriendyType
                            });
                            break;
                    }
                },
                ["group list"] = a => player.Reply(string.Join(", ", _chatGroups.Select(g => g.GroupName))),
            ...
            command.Value(remainingArgs.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries));
        }
        ...
        private IPlayer FindPlayer(string nameOrID, out string response)
        {
            ...
            {
                IPlayer result = players.All.FirstOrDefault(p => p.Id == nameOrID);
            ...
            {
                if (string.Equals(current.Name, nameOrID, StringComparison.OrdinalIgnoreCase))
                    return current;

                if (current.Name.IndexOf(nameOrID, StringComparison.OrdinalIgnoreCase) >= 0)
            ...
            switch (foundPlayers.Count)
            {
                ...
                default:
                    response = "Multiple matching players found: \n" + string.Join(", ", foundPlayers.Select(current => current.Name));
                    break;
            }
        ...
        private void LoadData<T>(ref T data, string filename = null)
        {
            try
            {
                data = Core.Interface.Oxide.DataFileSystem.ReadObject<T>(filename ?? Name);
            }
            catch
            {
        ...
        private static string StripRichText(string text)
        {
            ...
            foreach (string replacement in _stringReplacements)
                text = text.Replace(replacement, string.Empty);

            foreach (Regex replacement in _regexReplacements)
        ...
        public class BetterChatMessage
        {
            ...
            public ChatGroup.FormatSettings GetOutput()
            {
                ...
                if (!string.IsNullOrEmpty(Message) && (Message.Contains("[#") || Message.Contains("[+")))
                ...
                Dictionary<string, string> replacements = new Dictionary<string, string>(7)
                {
        ...
        public class ChatGroup
        {
            ...
            public static List<ChatGroup> GetUserGroups(IPlayer player)
            {
                ...
                List<ChatGroup> groups = _instance._chatGroups.Where(g => oxideGroups.Any(name => g.GroupName.Equals(name, StringComparison.OrdinalIgnoreCase))).ToList();
            ...
            private static ChatGroup GetPrimaryGroup(List<ChatGroup> groups)
            {
                ChatGroup primary = null;

                for (int i = 0; i < groups.Count; i++)
                {
                    ChatGroup group = groups[i];
            ...
            {
                List<ChatGroup> groups = GetUserGroups(player);
                ChatGroup primary = GetPrimaryGroup(groups);
                ...
                List<string> titles = new List<string>(Math.Min(groups.Count, _instance._config.MaxTitles + _instance._thirdPartyTitles.Count));

                for (int i = 0; i < groups.Count; i++)
                {
                    ChatGroup group = groups[i];
                    if (group.Title.Hidden || (group.Title.HiddenIfNotPrimary && primary != group))
                        continue;

                    titles.Add($"[#{group.Title.GetUniversalColor()}][+{group.Title.Size}]{group.Title.Text}[/+][/#]");

                    if (titles.Count >= _instance._config.MaxTitles)
                        break;
                }
                ...
                foreach (KeyValuePair<Plugin, Func<IPlayer, string>> thirdPartyTitle in _instance._thirdPartyTitles)
                {
                    try
                    {
